...
- 結果：カードを少し横に動かすだけで、LIKEまたはNOPEとして分類されるようになった。
- わかったこと：swipeThresholdの値を小さくするとスワイプが成立しやすくなり、大きくすると長い距離を動かさなければ成立しないことがわかった。

**実験2：**
- やったこと：カードの回転角度を計算する式を、20で割る形から10で割る形に変更した
  rotation = Double(value.translation.width / 10)
- 結果：同じ距離をドラッグしても、変更前よりカードが大きく傾くようになった。
- わかったこと：割る数を小さくすると回転角度が大きくなり、カードの動きが強調されることがわかった。ただし、回転が大きすぎるとカードが見にくくなるため、適切な調整が必要である。

## AIに聞いて特に理解が深まった質問 TOP3

1. **質問：DragGestureの.onChangedと.onEndedには、どのような違いがあるのか。**
   **得られた理解：.onChangedはドラッグ中に繰り返し呼ばれ、カードの位置や回転を更新するために使う。.onEndedは指を離したときに呼ばれ、スワイプの成否や方向を判断するために使う。**

2. **質問：なぜスワイプの判定にswipeThresholdが必要なのか。**
   **得られた理解：一定の移動距離を超えた場合だけスワイプを成立させることで、少し触れただけの操作が誤ってLIKEやNOPEになることを防げる。値を小さくすると反応しやすく、大きくすると慎重な操作が必要になる。**

3. **質問：なぜカードを削除する前にasyncAfterで0.3秒待つのか。**
   **得られた理解：最初にカードを画面外まで移動させ、そのアニメーションが終わってから配列から削除するためである。すぐに削除するとカードが突然消え、退場するアニメーションが見えなくなる。**

## この章のまとめ

この章では、DragGestureを使ってカードのドラッグ操作を検出し、移動距離をoffset、回転角度をrotationで管理する方法を学んだ。.onChangedではドラッグ中の表示を更新し、.onEndedでは移動距離とswipeThresholdを比較して、左右のスワイプを判定する。
また、.easeOutと.springを使い分けることで、カードが画面外へ移動する動きや、元の位置へ戻る動きを自然に表現できる。ジェスチャー、状態管理、アニメーションを組み合わせることで、操作に反応するTinder風のカードUIを実装できる。
