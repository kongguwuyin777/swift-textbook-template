...
音楽検索アプリの画面を作成し、検索・一覧表示・詳細表示・エラー表示を行っている。

**なぜこう書くのか：**

Viewごとに役割を分けることで読みやすくなり、状態に応じて画面を切り替えやすくなるため。

**もしこう書かなかったら：**

コードが複雑になり、保守しにくくなる。

また、読み込み中・エラー・詳細画面遷移などの表示が適切に行えなくなる。
...
**実験1：**
- やったこと：検索ボタンだけでなく、キーボードのReturnキーでも検索できるようにした。
- TextField("アーティスト名を入力", text: $searchText)
    .textFieldStyle(.roundedBorder)
    .onSubmit {
        Task {
...
- 結果：文字を入力してReturnキーを押すと、検索ボタンを押さなくても検索できた。
- わかったこと：onSubmit を使うと、TextFieldで入力完了した時の処理を追加できることがわかった。

**実験2：**
- やったこと：空白だけを入力した場合は検索できないようにした。
- .disabled(searchText.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty)
- 結果：空白だけでは検索ボタンが押せなくなった。
- わかったこと：trimmingCharacters を使うと、文字列の前後の空白を取り除いて判定できることがわかった。

## AIに聞いて特に理解が深まった質問 TOP3

1. **質問：iTunes Search APIを使って音楽検索アプリを作るにはどうすればよいか**
   **得られた理解：APIからJSONデータを取得し、CodableでSwiftのデータ型に変換して画面に表示する流れを理解した。**

2. **質問：async/awaitを使ったAPI通信はどのように書くのか**
   **得られた理解：Taskで非同期処理を開始し、URLSession.shared.data(from:)でデータを取得できることが分かった。**

3. **質問：検索結果をSwiftUIのリストに表示するにはどうすればよいか**
   **得られた理解：ListとIdentifiableを使うことで、APIから取得した曲データを一覧表示できることを理解した。**
