...
1. **質問：**
   なぜMapで cameraPosition を管理する必要があるのか。
   **得られた理解：**
    cameraPosition を使うことで、地図の中心位置やズーム範囲をSwiftUI側で管理できることが分かった。
2. **質問：**
   なぜ Set を使ってカテゴリを管理しているのか。
   **得られた理解：**
    Set を使うことで、カテゴリの重複を防ぎながら、追加・削除を効率よく行えることが分かった。
3. **質問：**
   Marker と ForEach を組み合わせる理由は何か。
   **得られた理解：**
    ForEach を使うことで、複数の観光スポットをまとめて繰り返し表示でき、データ数が増えても簡単に管理できることが分かった。
## この章のまとめ

この章では、MapKitを使って地図を表示し、観光スポットをマーカーとして配置する方法を学んだ。
また、MapCameraPositionによる地図の表示制御や、カテゴリごとのフィルター機能の実装方法について理解を深めることができた。
さらに、SwiftUIのMap、Marker、ForEach、Setなどを組み合わせることで、地図アプリの基本的な構成を実践的に学ぶことができた。
