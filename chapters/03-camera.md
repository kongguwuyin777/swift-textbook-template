...
**実験1：**
- やったこと：imageDisplayAreaのframe(maxHeight: 400)を300に変更した。
- 結果：表示される写真のサイズが小さくなった。
- わかったこと：frameを変更することで画像の表示サイズを調整できることが分かった。

**実験2：**
- やったこと：プレースホルダーのImage(systemName: "photo")を"camera"に変更した。
- 結果：画像未選択時に表示されるアイコンがカメラに変わった。
- わかったこと：SF Symbolsを変更するだけで画面の見た目を簡単に変更できることが分かった。

## AIに聞いて特に理解が深まった質問 TOP3

1. **質問：Coordinatorパターンはなぜ必要なのか。**
   **得られた理解：UIKitのDelegate処理とSwiftUIを連携するために使用する。撮影した写真をSwiftUI側へ渡す役割があることを理解した。**

2. **質問：なぜ画像を非同期で読み込むのか。**
   **得られた理解：大きな画像を読み込む際に画面が固まるのを防ぐためであり、async/awaitを利用することで快適な操作を実現できることが分かった。**
