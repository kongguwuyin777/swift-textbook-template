...
**実験1：**
- やったこと：一覧画面の並び順を .reverse から .forward に変更した。
  @Query(
    sort: \PhotoRecord.createdAt,
    order: .forward
) private var records: [PhotoRecord]
- 結果：新しい記録が上に表示されるのではなく、古い記録から順番に表示された。
- わかったこと：@Query の order を変更すると、SwiftDataから取得するデータの表示順を簡単に変更できることがわかった。

**実験2：**
- やったこと：保存ボタンの無効化条件から、位置情報の確認を一時的に外して試した。
  .disabled(title.isEmpty)
- 結果：位置情報を取得していなくても保存ボタンを押せるようになった。しかし、saveRecord() の guard で処理が終了するため、記録は保存されなかった。
- わかったこと：.disabled は利用者の誤操作を防ぎ、guard はプログラム側で不正なデータの保存を防いでいることがわかった。両方を使うことで、より安全な処理になる。

## AIに聞いて特に理解が深まった質問 TOP3

1. **質問：SwiftDataでは、なぜ画像をUIImageではなくData型で保存するのか。**
   **得られた理解：SwiftDataにはUIImageをそのまま保存できないため、画像を保存可能なData型に変換する必要がある。表示するときは、保存したDataを再びUIImageへ変換する。**

2. **質問：.disabledとguardは、なぜ両方必要なのか。**
   **得られた理解：.disabledは、条件を満たしていないときに保存ボタンを押せなくし、利用者の誤操作を防ぐ。一方、guardはプログラム内部で条件を再確認し、不完全な記録が保存されることを防ぐ。両方を使うことで、安全性が高くなる。**

3. **質問：写真と位置情報は、どのように関連付けて保存されているのか。**
   **得られた理解：選択した画像データと、現在地の緯度・経度を同じPhotoRecordに格納している。そのため、保存後も写真と撮影場所の関係が維持され、地図上の正しい位置に写真を表示できる。**

## この章のまとめ
この章では、SwiftUIでカメラ・写真、位置情報、地図、SwiftDataを組み合わせる方法を学んだ。写真はData型に変換し、タイトル、メモ、緯度・経度、作成日時と一緒にPhotoRecordとして保存する。
@Modelでデータモデルを定義し、@Queryで保存済みの記録を取得できる。位置情報はCLLocationManager、写真の選択はPhotosPicker、地図表示にはMapとAnnotationを使用する。
最も重要なのは、それぞれの機能を別々に使うだけでなく、1件のデータとして関連付けることである。これにより、写真を撮影場所とともに保存し、地図や一覧画面から確認できるアプリを作成できる。
